...
export default function LoginButton() {
            ...
            <span className="group relative">
              {isLoading ? (
                ...
                <SignOut
                  ...
                  color="red"
                  weight="fill"
                  className="cursor-pointer rounded-full p-1 transition-colors duration-200 hover:bg-zinc-50 dark:hover:bg-zinc-800"
                />
              )}
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="flex max-w-max flex-row items-center justify-center rounded-lg border border-zinc-300 bg-white px-6 py-3 shadow-md transition-shadow hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-700 dark:bg-black dark:text-white dark:hover:bg-zinc-800 dark:hover:shadow-zinc-700"
    >
      {isLoading ? (
        <Spinner className="mr-3 h-5 w-5 animate-spin text-zinc-400 dark:text-zinc-600" />
      ) : (
